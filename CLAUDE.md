...
- **Supabase 인증**: 로그인으로 게임 상태 영구 저장 (선택사항)
...
### devchat invite - 초대 코드 생성

친구를 초대하기 위한 초대 코드를 생성합니다. 초대 코드에는 서버 주소, 포트, Supabase 인증 정보가 포함됩니다.
...
# 인증 정보 없이 생성
devchat invite --no-auth
```

출력된 초대 코드를 친구에게 공유하면, 친구는 `devchat join --invite <코드>`로 접속할 수 있습니다.
...
# 초대 코드로 접속 (권장)
devchat join --invite <초대코드> --nick "박코딩"
```
...
| `--port <port>` | 기본 포트 설정 |
| `--supabase-url <url>` | Supabase 프로젝트 URL |
...
가장 쉬운 방법입니다. 호스트가 초대 코드를 생성하면 친구는 한 줄 명령어로 접속합니다.

```bash
# 터미널 1: 호스트가 방 생성
devchat host --port 8080 --name "우리팀방"
...
#   eyJob3N0IjoiMTkyLjE2OC4xLjEwMCI...
#
# They can join with:
#   devchat join --invite eyJob3N0IjoiMTkyLjE2OC4xLjEwMCI... --nick "YourName"
```

친구에게 출력된 명령어를 공유하면 됩니다. 초대 코드에는 서버 주소, 포트, 인증 설정이 모두 포함됩니다.
...
Supabase 인증을 설정하면 사용자별 게임 상태가 영구 저장됩니다. 재접속해도 RPG 캐릭터 레벨, 골드 등이 유지됩니다.
...
초대 코드에 인증 설정이 자동 포함됩니다:

```bash
devchat invite --port 8080
# 출력된 초대 코드를 친구에게 공유

# 친구
devchat join --invite <코드> --nick "친구"
```

**방법 2: 수동 설정**

호스트의 Supabase URL과 Key를 친구에게 공유:

```bash
# 친구가 직접 설정
devchat config --supabase-url "호스트의-url"
devchat config --supabase-key "호스트의-key"
```

### 로그인/회원가입

Supabase가 설정된 상태에서 `devchat`을 실행하면 로그인 화면이 표시됩니다.

- 이메일/비밀번호로 회원가입 및 로그인
- 비밀번호: 8자 이상, 대문자, 소문자, 숫자 포함
- "Skip" 선택 시 인증 없이 사용 가능 (게임 상태 저장 안됨)

### UID 기반 상태 관리

인증된 사용자는 고유 UID로 식별됩니다:

- 닉네임을 변경해도 동일 계정으로 인식
- 방을 다시 열어도 이전 게임 상태 복원
- 다른 기기에서 접속해도 동일 계정 사용 가능
